...
def objective(config):
    ...
    input_config = copy.deepcopy(config_object)
    input_config.model.n_layers = config['n_layers']
    input_config.model.d_model = config['d_model']
    input_config.model.layer.n_ssm = config['d_model']
    input_config.model.prenorm = config['prenorm']
    input_config.model.layer.d_state = config['d_state']
    input_config.model.layer.lr = config['lr']
    input_config.model.layer.bidirectional = config['bidirectional']
    input_config.optimizer.lr = config['optimizer_lr']

    output_list = train(input_config)
    print(f"OBJECTIVE OUTPUT: {output_list}")
    return output_list[1]['final/test/loss']
...
def main(config: OmegaConf):
    ...
    config_object = utils.train.process_config(config)

    # Pretty print config using Rich library
    # utils.train.print_config(config_object, resolve=True)

    search = pyhopper.Search(
        {
            "n_layers": pyhopper.int(1,10,6),
            "d_model": pyhopper.int(32,256,128),
            "prenorm": pyhopper.choice([True,False],0),
            "d_state": pyhopper.int(8,256,256),
            "lr": pyhopper.float(1e-2,1e-4,"0.1g", 0.001),
            "bidirectional": pyhopper.choice([True,False], 0),
            "optimizer_lr": pyhopper.float(1e-2,5e-3,"0.1g", 0.005)
        }
    )

    best_params = search.run(objective, "minimize", "3day",n_jobs="1",ignore_nans=True)

    print(f"{config_object.dataset.target}: {search.best_f} {best_params}")
    with open(f"{config_object.dataset.target}_search_results.txt", "a") as f:
        f.write(f"Best loss: {search.best_f}\n")
        f.write(f"Best params: {best_params}\n")
        f.write("\n")
    search.save(f"{config_object.dataset.target}_search_save.out")
